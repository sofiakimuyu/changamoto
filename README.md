...
The finish screen's **Sign up to track progress** is passwordless: the player
enters an email, Supabase Auth emails a **six-digit code**, and typing it back
signs them in. Their account id then becomes their leaderboard identity so
progress follows them across devices; when signed out (or with no backend) an
anonymous per-device id is used instead.
